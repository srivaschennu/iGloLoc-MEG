function [model,family] = garridomodels_new(convec)
...
m=1;
model(m) = initmodel(locs([1 2],:),convec);
model(m).C(1:2) = 1;
...
% Add STG
m = 3;
model(m) = initmodel(locs(1:4,:),convec);
model(m) = copymodel(model(1),model(m));
...
% Add RIFG
m = 5;
model(m) = initmodel(locs(1:5,:),convec);
model(m) = copymodel(model(3),model(m));
...
m = 9;
model(m) = initmodel(locs([1:4 7 8],:),convec);
model(m) = copymodel(model(3),model(m));

model(m).A{1}(6,4) = 1; % RSTG forward connection on RIPC
model(m).A{2}(4,6) = 1; % RIPC backward connection on RSTG
model(m).B{1}(6,4) = 1; % RSTG forward modulation on RIPC
model(m).B{1}(4,6) = 1; % RIPC backward modulation on RSTG

model(m).A{1}(5,3) = 1; % LSTG forward connection on LIPC
model(m).A{2}(3,5) = 1; % LIPC backward connection on LSTG
model(m).B{1}(5,3) = 1; % LSTG forward modulation on LIPC
model(m).B{1}(3,5) = 1; % LIPC backward modulation on LSTG

model(m).A{3}(6,5) = 1; % LIPC lateral connection on RIPC
model(m).A{3}(5,6) = 1; % RIPC lateral connection on LIPC
model(m).B{1}(6,5) = 1; % LIPC lateral modulation on RIPC
model(m).B{1}(5,6) = 1; % RIPC lateral modulation on LIPC
...
m = 11;
model(m) = initmodel(locs([1:5 7 8],:),convec);
model(m) = copymodel(model(5),model(m));

model(m).A{1}(7,5) = 1; % RIFG forward connection on RIPC
model(m).A{2}(5,7) = 1; % RIPC backward connection on RIFG
model(m).B{1}(7,5) = 1; % RIFG forward modulation on RIPC
model(m).B{1}(5,7) = 1; % RIPC backward modulation on RIFG

model(m).A{3}(7,6) = 1; % LIPC lateral connection on RIPC
model(m).A{3}(6,7) = 1; % RIPC lateral connection on LIPC
model(m).B{1}(7,6) = 1; % LIPC lateral modulation on RIPC
model(m).B{1}(6,7) = 1; % RIPC lateral modulation on LIPC
...
% Add LIFG
m = 13;
model(m) = initmodel(locs(1:8,:),convec);
model(m) = copymodel(model(7),model(m));
